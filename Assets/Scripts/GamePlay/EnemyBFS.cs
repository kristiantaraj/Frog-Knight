...
public class EnemyBFS : MonoBehaviour
{
    ...
    void Update()
    {
        startPos = tilemap.WorldToCell(enemy.transform.position);
...
float f1(Vector3Int node)
{
    ...
    void MoveEnemy(Vector3Int[] path)
    {
        if (path != null && path.Length > 0)
        {
            // Check if the enemy has reached the current node on the path
            Vector3 targetPos = grid.CellToWorld(path[currentPathIndex]);
            if (enemy.transform.position == targetPos)
            {
                currentPathIndex++;
                if (currentPathIndex >= path.Length)
                {
                    // End the movement if the enemy has reached the goal
                    return;
                }
            }
            // Move the enemy one tile towards the next node on the path
            enemy.transform.position = Vector3.MoveTowards(enemy.transform.position, targetPos, Time.deltaTime * speed);
        }
    }
